...
public class SetGame {
	...
	/*
	 * Initializes the players and card array as well as creating a pcs for sending signals for changes
	 */
	public SetGame() {
	...
	public synchronized void removePlayer(String player, byte id) {
		...
		if (temp != null) {
			...
			numPlayersRemoved++;
			if (numPlayersRemoved >= numPlayers) {
				gameRunning = false;
			}
		}
	...
	public synchronized void drawCards(byte playerNum){
		System.out.println(playerNum + "wants to draw cards");
		if (!playersAskingDraw.contains(playerNum)) {
			playersAskingDraw.add(playerNum);
		}
		if (playersAskingDraw.size()<=(numPlayers-numPlayersRemoved)/2) {
			return;
		}
	...
	public short[] getScores() {
		short[] scores = new short[numPlayers];
		for (Player p : players) {
